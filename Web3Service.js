...
export default class Web3Service {
    ...
    async connect(which) {
      ...
      runInAction(() => {
        this.netId = netId;
        if (netId === 1) { this.network = 'Mainnet'; } else if (netId === 3) { this.network = 'Ropsten'; } else if (netId === 4) { this.network = 'Rinkeby'; } else { this.network = 'Private'; }
      });
    ...
    async deploy(contractData) {
      const { web3, deployerInstance } = this;
    ...
    async requestFromFaucet() {
      ...
      if (tokenBalance < MIN_FEE) { return { status: -1 }; }

      if ((now - lastRequest) < waitTime) { return { status: 0, data: waitTime - (now - lastRequest) }; }
    ...
    convertMiningPower = (value, reverse) => {
      if (reverse) { return this.convertEtherToWei(value, true) * 100; }
      return this.convertEtherToWei(value) / 100;
    }

    convertEtherToWei = (value, reverse) => {
      if (reverse) { return value / 1e+18; }
      return value * 1e+18;
    }

    finePrint = (value) => {
      if (new RegExp('^\\d+\\.?\\d{8,}.*$').test(value)) { return Number(value).toFixed(8); } else { return value; }
    }
    ...
    async checkTokenOwnership(contract) {
      ...
      if (owner == web3.eth.defaultAccount) { return owner; } else if (owner != web3.eth.defaultAccount && newOwner == web3.eth.defaultAccount) { return true; } else { return false; }
    }
    ...
    async fetchReceipt(hash) {
      const { web3 } = this;
      const receipt = await Bb.fromCallback(callback =>
    ...
    async fetchNewChild(hash) {
      ...
      let foundLog;
      if (!receipt.logs || typeof receipt.logs !== 'object' || receipt.logs.length < 1) { return false; }
      receipt.logs.forEach((l) => {
        if (l.address.toLowerCase() === DEPLOYER_ADDRESS.toLowerCase()) {
          if (l.topics[0] == config.topic) { foundLog = l.data; }
        }
      });
      if (!foundLog) { return false; }
      const result = ethers.Interface.decodeParams(config.params, foundLog);
    ...
    async fetchCreationHash(contract) {
      ...
      created.forEach((log) => {
        if (log.args[config.eventContractTitle].toLowerCase() == contract.toLowerCase()) { foundLog = log.transactionHash; }
      });
      return foundLog;
    }
    ...
    parseAllocationLog = (logs) => {
      const found = [];
      logs.map((l) => {
        for (const m in l.args) {
          if (typeof l.args[m].valueOf !== 'undefined') { l.args[m] = l.args[m].valueOf(); }
          switch (true) {
    ...
    async getAllocationHistory(contract) {
      ...
      return allocations.sort((a, b) => a.blockNumber - b.blockNumber);
    }
