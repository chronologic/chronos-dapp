      ...
      case 'chronos':
        ...
        case 'debt':
          ...
          data = {
            ...
            lender: (await Bb.fromCallback( callback => debtContract.debtOwner.call(callback) )).valueOf(),
            borrower: (await Bb.fromCallback( callback => debtContract.owner.call(callback) )).valueOf(),
...
g
  ...
  async getDeploymentData(transaction){
    let { web3 } = this;
    const data ={
      creationCode: (await Bb.fromCallback( callback => web3.eth.getTransaction(transaction, callback))).input ,abi: JSON.stringify( dayTokenABI ),
    }
    return data;
  }
}
